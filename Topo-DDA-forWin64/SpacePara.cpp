...
SpacePara::SpacePara(Vector3i bind_, StructureAndSpace* structureandspace_, VectorXd* Inputdiel, bool Filter_, FilterOption* Filterstats_, string symmetry, vector<double> symaxis, bool Periodic_, int Lx_, int Ly_) {
    Filter = Filter_;
    structureandspace = structureandspace_;
    bind = bind_;
    int Nx, Ny, Nz, N;
    tie(Nx, Ny, Nz, N) = ( *structureandspace ).get_Ns( );
    geometry = *(*structureandspace).get_geometry();
    Periodic = Periodic_;
    Lx = Lx_;
    Ly = Ly_;
    if ( Filter && ( bind(0) != 1.0 ) || bind(1) != 1.0 ) {
...
StructureAndSpace* SpacePara::get_structureAndSpace() {
    return structureandspace;
}
