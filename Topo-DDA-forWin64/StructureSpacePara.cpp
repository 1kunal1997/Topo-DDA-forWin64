...
StructureSpacePara::StructureSpacePara(int Nx_, int Ny_, int Nz_, int N_, VectorXi* geometry_, VectorXd* Inputdiel, bool Filter_, FilterOption* Filterstats_, string symmetry, vector<double> symaxis, bool Periodic_, int Lx_, int Ly_) {
	Nx = Nx_;
	Ny = Nz_;
	Nz = Nz_;
	N = N_;
    ...
    int NFpara = int(round(( int(geometry.size( )) / 3 / Nz / dividesym )));     // number of pixels in xy plane divided by symmetry 
    ...
    if ( Filter == true ) {
        ...
        FreeWeight = vector<vector<WeightParas>>(NFpara);
                ...
                for ( int k = 0; k < Paratogeometry[ j ].size( ); k++ ) {
                    ...
                    if ( Periodic == false ) {
                        if ( ( zo == zr ) && ( circlerange(xo, yo, xr, yr, rfilter) ) ) {
                            ...
                            FreeWeight[ i ].push_back(WeightParas{ weight,posweight });
                            //break;
...
void StructureSpacePara::ChangeFilter( ) {
    ...
    vector<vector<WeightParas>> FreeWeight_tmp(NFpara);
    for ( int i = 0; i <= NFpara - 1; i++ ) {
        int poso = Paratogeometry[ FreeparatoPara(i) ][ 0 ];                 //As 2D extrusion is assumed, different z does not matter
        int xo = geometry(3 * poso);
        int yo = geometry(3 * poso + 1);
            ...
            for ( int k = 0; k < Paratogeometry[ j ].size( ); k++ ) {
                ...
                if ( Periodic == false ) {
                    if ( ( zo == zr ) && ( circlerange(xo, yo, xr, yr, rfilter) ) ) {
                        ...
                        int posweight = j;
                        double weight = calweight(xo, yo, xr, yr, rfilter);
                        FreeWeight_tmp[ i ].push_back(WeightParas{ weight,posweight });
                        //break;
                        //As soon as one in the entire z direction is verified, no need for looking at others for 1 j. But when there is symmetry, this is needed because same z can have differnt x, y.
                    }
                }
...
vector<vector<WeightParas>>* StructureSpacePara::get_FreeWeight( ) {
    if ( !Filter ) {
