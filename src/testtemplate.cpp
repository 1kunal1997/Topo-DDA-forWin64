...
int main() {
    ...
    string save_position = ".\\ccylinder-allWater-TR-verify\\";       //output file
    Vector3d l;
    Vector3d center;
    l << 19.0, 19.0, 37.0;    //Size of the initialization block. 81*81*17 pixels in total.
    ...
    l1 << 19.0, 19.0, 19.0;
    center1 << l1(0) / 2, l1(1) / 2, 27.5;
    l2 << 19.0, 19.0, 7.0;
    center2 << l2(0) / 2, l2(1) / 2, 13.5;
    l3 << 19.0, 19.0, 9.0;
    center3 << l3(0) / 2, l3(1) / 2, 4.5;

    Vector3d center_rect;
    double radius = 10;
    double cylinh = 19.0;
    center_rect << l1(0) / 2, l1(1) / 2, 27.5;
    Structure s1_rect_start(S.get_total_space(), radius, cylinh, center_rect);
    ...
    SpacePara spacepara(&S, bind, vector<string>{}, vector<double>{1.0, 0.0, 0.0, 0.0});
    ...
    for (list<double>::iterator itlam = lam_l.begin(); itlam != lam_l.end(); itlam++) {
        double lam = *itlam;
        list<string> mat_l{ "H2O", "TiN" };
        VectorXcd material = Get_X_material(mat_l, lam, "nm");
        double nback = sqrt(real(material(0)));
        AProductCore Core(&CStr, lam, material, nback, m, n, Lm, Ln, "FCD");
        DDAModel TestModel(&Core, n_K, E0, n_E0);
        TestModel.bicgstab(MAX_ITERATION_DDA, MAX_ERROR);
        TestModel.update_E_in_structure();
        TestModel.solve_E();
...
int main() {
    ...
    string save_position = ".\\import99-TR-verify\\";       //output file
    Vector3d l;
    Vector3d center;
...
int main() {
    ...
    string save_position = ".\\cylinder-TR-verify\\";       //output file
    Vector3d l;
    Vector3d center;
    l << 19.0, 19.0, 29.0;    //Size of the initialization block. 81*81*17 pixels in total.
    center << l(0) / 2, l(1) / 2, l(2) / 2;      //Center of the block.
    ...
    bool Filter = false;
    double r_f = 2.0;
    vector<filterinfo> filterlist{ filterinfo{0,1.0}, filterinfo{40,2.0} };
    FilterOption filteropt(0.0, 50.0, 0.5, "piecewise", filterlist);
    string symmetry = "4fold";

    SpacePara spacepara(&S, bind, vector<string>{}, vector<double>{0.0, 1.0, 1.0, 2.0}, Filter, & filteropt);
...
int main() {
    ...
    string save_position = ".\\thick400-diel2d5-phi0theta0-lam500-size1000-focus450-2D\\";       //output file
    Vector3d l;
    Vector3d center;
    l << 80.0, 80.0, 16.0;    //Size of the initialization block. 81*81*17 pixels in total.
    center << l(0) / 2, l(1) / 2, l(2) / 2;      //Center of the block.
    ...
    bool Filter = true;
    double r_f = 2.0;
    vector<string> FParaInit{ "ZEROS" };
    vector<filterinfo> filterlist{ filterinfo{0,1.0}, filterinfo{20,2.0} };
    FilterOption filteropt(0.0, 80.0, 0.5, "piecewise", filterlist);
    SpacePara spacepara(&S, bind, FParaInit, FPGeometryl, BPGeometryl, BParal, Filter, &filteropt);                              //Initialize with material index of 1, which is permittivity=2.5 in this case.
                                                                        //SpacePara is where the parameter<->geometry link is established.
    list<string> ObjectFunctionNames{ "PointE" };                       //Name of the object function.
    list<list<double>*> ObjectParameters{ &ObjectParameter };
    list<DDAModel> ModelList;
    ...
    AProductCore Core1(&CStr, lam(0), material, nback, "LDR");                //Matrix vector product is carried out in AProductCore class. So in this step, wavelength and actual permittivity comes in.
    CorePointList.push_back(&Core1);
    ofstream Common;
    Common.open(save_position + "Commondata.txt");
    Common << CStr.get_Nx() << endl << CStr.get_Ny() << endl << CStr.get_Nz() << endl << CStr.get_N() << endl;
...
int main() {
    ...
    SpacePara spacepara(&S, bind, FParaInit, BParal, Filter, &filteropt, symmetry, symaxis);
    //SpacePara is where the parameter<->geometry link is established.
...
int main() {
    ...
    string save_position = ".\\Absbyfar-fromit99-sym-filter2\\";       //output file
    Vector3d l;
    Vector3d center;
    l << 19.0, 19.0, 29.0;    //Size of the initialization block. 81*81*17 pixels in total.
    center << l(0) / 2, l(1) / 2, l(2) / 2;      //Center of the block.
